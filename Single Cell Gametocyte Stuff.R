...
# Convert the assay to a CsparseMatrix
...
# View results
head(deg_results)
# View top 10 upregulated genes (highest log2 fold change)
top_up <- head(deg_results[order(-deg_results$avg_log2FC), ], 10)
print("Top Overexpressed Genes:")
print(top_up)

# View top 10 downregulated genes (lowest log2 fold change)
...
Dogga_early_gametocytes <- FindClusters(Dogga_early_gametocytes, resolution = 0.25)  # Adjust resolution for finer or coarser clusters
...
# Male and female specific genes clustering
...
dev.off() 
# Calculate the average expression of male and female genes for each cell
