...
Dd2_data_gametocytes <- FindClusters(Dd2_data_gametocytes, resolution = 0.25)  # Adjust resolution for finer or coarser clusters
...
# Save it as an interactive HTML widget file
...
for (curve in slingshot_data@curves) {
  ...
  pca_embeddings <- reducedDim(Dd2_sce, "PCA")  # Get PCA embeddings
...
counts <- counts(Dd2_sce)  # Raw counts

cell_weights <- slingshot::slingCurveWeights(Dd2_sce)  # Cell weights
...
# Prepare data for tradeSeq
Dd2_sex_counts <- counts(Dd2_sex_sce)  # Raw counts

Dd2_sex_cell_weights <- slingshot::slingCurveWeights(Dd2_sex_sce)  # Cell weights
